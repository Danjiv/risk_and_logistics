...
def SCENARIOS_model(Candidates, Times, Suppliers, Products,Customers, Scenarios,
    ...
    supply = np.array([prob.addVariable(name='supply_{0}_{1}_{2}_{3}'.format(c, s, t, sc), vartype=xp.integer)
    ...
    warehoused = np.array([prob.addVariable(name='warehoused_{0}_{1}_{2}_{3}'.format(c, p, t, sc), vartype=xp.integer)
    ...
    prob.setObjective(xp.Sum(open[c-1,t-1]*Operating_df["Operating cost"][c-1] for c in Candidates for t in Times) +
                    ...
                    xp.Sum(supply[c-1, s-1, t-1, sc-1]*CostSupplierCandidate[(s, c)] 
    ...
    prob.addConstraint(xp.Sum(supply[c-1, s-1, t-1, sc-1] for c in Candidates) <= Suppliers_df["Capacity"][s]
                        for s in Suppliers for t in Times for sc in Scenarios)
    # update warehouse stock
    prob.addConstraint(warehoused[c-1, p-1, t-1, sc-1] == xp.Sum(supply[c-1, s-1, t-1, sc-1] 
